...
main = print . Prelude.length . blocks . run M.empty . execute . parse 10000 =<< getContents
    where blocks = filter (==Block) . M.elems

data Tile = Empty | Wall | Block | Paddle | Ball deriving (Show, Eq)

run :: Grid -> Effect -> Grid
run grid effect =
    let toTile = (!!) [Empty,Wall,Block,Paddle,Ball] . narrow
    in case effect of
         Input whatColor             -> error "No input for part one"
         Output x (Output y (Output tile fx)) -> flip run fx $ M.insert (x,y) (toTile tile) grid
         End                         -> grid
